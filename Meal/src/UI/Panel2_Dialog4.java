...
public class  Panel2_Dialog4 extends JDialog{
	...
	class Button_Panel extends JPanel{
		...
		public void paintComponent(Graphics g) {
			...
			g.drawString("양식 판매량 TOP3 차트", 100, 25);
		}
	}
	...
	public void open_chart() {
		...
		try {
			...
			PreparedStatement ps = con.prepareStatement("SELECT sum(orderCount) AS CNT FROM orderlist WHERE cuisineNo=4");
			...
			PreparedStatement ps1 = con.prepareStatement("SELECT mealNo, sum(orderCount) AS SUM_ORDER FROM orderlist WHERE cuisineNo=4 GROUP BY mealNo ORDER BY sum(orderCount) desc limit 3");
